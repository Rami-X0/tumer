...
import 'package:tumer_detection/features/home/ui/patient/get_all_message_patient_screen.dart';

class GridPatient extends StatefulWidget {
  const GridPatient({super.key});

  @override
  State<GridPatient> createState() => _GridPatientState();
}

class _GridPatientState extends State<GridPatient> {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<DoctorsCubit, DoctorsState>(
      buildWhen: (previous, current) =>
          current is GetAllPatientsLoadingState ||
          current is GetAllPatientsSuccessState,
      builder: (context, state) {
        ...
        } else if (state is GetAllPatientsSuccessState) {
          return _defaultGrid(state.patients);
        } else {
          return Center(
...
// print()
                          setState(() {});
                          print(
                              'ssssssssssssssssssss${context.read<DoctorsCubit>().receiverId}');
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (_) => GetAllMessagePatientScreen(
